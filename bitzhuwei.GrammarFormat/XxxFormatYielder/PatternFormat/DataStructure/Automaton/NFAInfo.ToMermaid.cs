...
namespace bitzhuwei.PatternFormat {
    ...
    partial class NFAInfo {
        ...
        private void PrintEdges(TextWriter w) {
            var queue = new Queue<NFAStateDraft>(); queue.Enqueue(this.start);
            var visitedEdges = new List<NFAEdgeDraft>();
            var visitedStates = new List<NFAStateDraft>();
            while (queue.Count > 0) {
        ...
        private void PrintStates(TextWriter w) {
            ...
            var queue = new Queue<NFAStateDraft>(); queue.Enqueue(this.start);
            var visited = new List<NFAStateDraft>();
            while (queue.Count > 0) {
                var state = queue.Dequeue();
