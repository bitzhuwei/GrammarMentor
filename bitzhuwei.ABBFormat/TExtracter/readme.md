# `ABB`的内容提取器（`Extracter`）

得到单词流`TokenList`和语法树`Node`后，就可以得到`ABB`的实际内容`BCounter`了。

实现`BCounterExtracter`的要点，简述如下：
...
4. 在不断经历上述2.和3.的过程中，`Node`中的信息会被不断提取到更高层的结点，最终提取到根结点，就形成了`BCounter`。

得到了`BCounter`，首先就可以格式化`ABB`的源文件了。这也可以验证前面的词法分析、语法分析、提取过程的正确性。

然后，之后程序员就可以自由地对此`BCounter`对象进行任何操作了。
