...
namespace Practice.GeneratedXxxFormat.Test {
    static partial class Test {
        public static void PrintCalc() {
            var compiler = new bitzhuwei.CalcFormat.CompilerCalc();

            Console.WriteLine("############ Processing: Calc ############");
            using (var w = new StreamWriter("Xxx/Calc.outputs")) {
                using (var reader = new StreamReader("Xxx/Calc.inputs")) {
                    while (!reader.EndOfStream) {
                        ...
                        tokens.Print(w);
                        w.WriteLine("```````````````````````````````");
                        node.Print(w, tokens, bitzhuwei.CalcFormat.CompilerCalc.Regulations);
                        w.WriteLine("```````````````````````````````");
