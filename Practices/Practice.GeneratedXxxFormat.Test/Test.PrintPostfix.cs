...
namespace Practice.GeneratedXxxFormat.Test {
    static partial class Test {
        public static void PrintPostfix() {
            var compiler = new bitzhuwei.PostfixFormat.CompilerPostfix();

            Console.WriteLine("############ Processing: Postfix ############");
            using (var w = new StreamWriter("Xxx/Postfix.outputs")) {
                using (var reader = new StreamReader("Xxx/Postfix.inputs")) {
                    while (!reader.EndOfStream) {
                        ...
                        tokens.Print(w);
                        w.WriteLine("```````````````````````````````");
                        node.Print(w, tokens, bitzhuwei.PostfixFormat.CompilerPostfix.Regulations);
                        //w.WriteLine("```````````````````````````````");
