...
namespace Practice.GeneratedXxxFormat.Test {
    static partial class Test {
        public static void PrintInts2() {
            var compiler = new bitzhuwei.Ints2Format.CompilerInts2();

            Console.WriteLine("############ Processing: Ints2 ############");
            using (var w = new StreamWriter("Xxx/Ints2.outputs")) {
                using (var reader = new StreamReader("Xxx/Ints2.inputs")) {
                    while (!reader.EndOfStream) {
                        ...
                        tokens.Print(w);
                        w.WriteLine("```````````````````````````````");
                        node.Print(w, tokens, bitzhuwei.Ints2Format.CompilerInts2.Regulations);
                        //w.WriteLine("```````````````````````````````");
