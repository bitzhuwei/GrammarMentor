...
namespace bitzhuwei.CalcFormat {
    partial class CompilerCalc {
        ...
        // TODO: optimize lexical analyzing like this:
        class LexicalTableItem {
            public readonly string[] beginTokens;

            public readonly LexicalState nextState;
        }
        private static readonly Action<LexicalContext, LexicalTableItem> PickTokenInfo =
        (context, tableItem) => {
            //BeginToken(context, tableItem.beginTokens);
            //ExtendToken(context);

            context.currentState = tableItem.nextState;
            context.MoveForward();
        };
